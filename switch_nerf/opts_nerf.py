...
def get_opts_base():
    ...
    parser.add_argument('--dataset_type', type=str, default='filesystem', choices=['filesystem', 'memory', 'blender', 'llff', 'bungee'],
    ...
    parser.add_argument('--grid_id', type=int, help='the grid index for current cluster')
    ...
    parser.add_argument('--llff_factor', type=int, default=1,
                        help='load the corresponding factor images for llff')
    ...
    parser.add_argument("--bungee_ray_nearfar", type=str, default='sphere', help='options: sphere/flat')
    parser.add_argument("--llffhold", type=int, default=8,
                        help='will take every 1/N images as LLFF or bungee test set, paper uses 8, bungee use 16')
    parser.add_argument('--num_epochs', type=int, default=10000,
                        help='number of training epochs')
    parser.add_argument('--colormap', type=int, default=4,
                        help='''colormap for depthmap in cv2, 4 means rainbow
...
def get_nerf_dataset_args(hparams):
    args = type('tmp', (object,), {})()
    args.logger = hparams.logger
    args.dataset_type = hparams.dataset_type
    args.datadir = hparams.dataset_path
    args.factor = hparams.llff_factor # downsample factor for LLFF images
    args.spherify = hparams.spheric_poses # for llff, not use
    args.llffhold = hparams.llffhold # for llff and bungee nerf
    args.no_ndc = hparams.dataset_type != 'llff' or hparams.no_ndc # ndc only for llff
    args.half_res = False # for blender
    args.testskip = hparams.testskip
    args.white_bkgd = hparams.white_bkgd
    args.scale_factor = hparams.scale_factor
    args.bungee_ray_nearfar = hparams.bungee_ray_nearfar
    return args